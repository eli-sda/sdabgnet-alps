...
import { Progress } from 'alps-library/molecules/components/progress/Progress';
...
const QuizDialog = ({ quizData, open, onClose }: QuizDialogProps) => {
  ...
  return (
    ...
    >
      <DialogTitle className="quiz-dialog-title">
        {showResults ? 'Резултат' : quizData.quizTitle}
      ...
      <DialogContent>
        {showResults ? (
          // RESULTS VIEW
          <div className="u-spacing--half u-space--top">
                    ...
                    <ul className="u-spacing--half">
                      {q.answers.map((ans, aIndex) => {
                        ...
                        const isSelected = isQMultiple
                          ? (userAnswer || []).includes(ansId)
                          : userAnswer === ansId;
                        const isCorrect = isQMultiple
                          ? q.correctAnswer.includes(ansId)
          ...
          // QUIZ VIEW
          <div className="u-spacing u-color--black u-space--top">
            <div className="u-spacing--half">
              <Progress percentage={progress} size="small" />
              <p className="u-color--black u-font-size--small">
                Въпрос {currentStep + 1} от {quizLength}
              </p>
            </div>
                  ...
                  <label key={index} className="quiz-answer-label">
                    {isMultiple ? (
                      ...
                      <input
                        ...
                        checked={isSelected}
                        onChange={() =>
                          handleSingleSelect(currentStep, answerId)
                        }
                      />
                    )}
